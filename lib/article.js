...
function getRandomDateWithinLast6Hours() {
    const now = new Date();
    const millisecondsIn24Hours = 6 * 60 * 60 * 1000;
    const randomMilliseconds = Math.floor(Math.random() * millisecondsIn24Hours);
    const randomDate = new Date(now - randomMilliseconds);
...
MultiEncoder.prototype = {
	...
	generate: function(data, pool, grabHeaders, obfuscateArticles) {
		this.part++;
		...
		if (obfuscateArticles) {
			const fileNameLength = this.fileNameLength !== undefined ? this.fileNameLength : (Math.floor(Math.random() * 17) + 16);
			this.fileNameLength = fileNameLength;
			filename = crypto.randomBytes(fileNameLength).toString('hex');
		}

		var yiPre = ' total='+this.parts+' line='+this.line_size+' size='+this.size+' name=', yiSuf = '\r\n=ypart begin=';
		...
		this.maxPostSize = y.maxSize(this.articleSize, this.line_size)
		...
		for(var h in this.headers) {
			...
			if(hl == 'subject' && v === null) {
				...
				if (obfuscateArticles && hl == 'date') {
					v = getRandomDateWithinLast6Hours();
				}
			}
	...
	createMessageId: function(part, parts, post) {
		return (''+this.messageIdFn.call(post, part, parts, post)).replace(RE_BADMSGIDCHAR, (Math.floor(Math.random() * 9) + 1).toString());
	}
};
...
Post.prototype = {
	...
	randomizeMessageID: function() {
		...
		if(newId.length > this.messageId.length)
			...
			newId = newId + '-' + this.messageId.substring(newId.length + 1).replace(/@/g, (Math.floor(Math.random() * 9) + 1).toString());
	...
	stripHeader: function(str) {
		...
		to = Buffer.byteLength(headers.substring(from, to), this.parent.encoding);
		from = Buffer.byteLength(headers.substring(0, from), this.parent.encoding);
		to += from;
...
UnpooledPost.prototype.releaseData = function() {
	this.bufs = [toBuffer(bufferSlice.call(this.data, 0, this.postPos))];
	this.release();
};
